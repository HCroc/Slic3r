...
# HOSP These are attributes and it states they are read/write and required.
...
use constant PILLAR_SIZE    => 2;
use constant PILLAR_SPACING => 3;
    ...
    my $process_layer = sub {
        ...
        if (@$base) {
            ...
            if ($layer_id == 0) {
                ...
                #HOSP These lines are those zigzags at the base of a model
                $density        = 0.5;
                ...
                #HOSP So here the width and height actually apply to the extrusion width and height
...
sub generate_pillars_shape {
    ...
    #HOSP So $contact is the total contact area for supports
    ...
    return if !%$contact;
     ...
     my $bb = Slic3r::Geometry::BoundingBox->new_from_points([ map @$_, map @$_, values %$contact ]);
        #HOSP Here the points are sent through the bounding box section to make sure they are in the area smaller then the
        #HOSP contact points I think.


        
        print STDERR "X_MAX";
        print STDERR $bb->x_max;
        my $x_max_halved = $bb->x_max;
        my $x_max_havled_main =$bb->x_max/2;
        print STDERR "X_MAX_HALVED";
        my $x_max_h_squared = sqrt($x_max_halved);
        print STDERR $x_max_halved;
        print STDERR "X_MAX_H_SQUARED";
        print STDERR $x_max_h_squared;
        my $x_max_pillar = $x_max_h_squared/1000;
        print STDERR $x_max_pillar;
    ...
    my $grid;  # arrayref of polygons
    {
        ...
        #HOSP So here in the my $pillar area you are creating a pillar with one point at 0,0 the next point at pillar_size
        #HOSP ,0 the next point at pillar_size, pillar size etc. These points are joined in the polygon section together by lines.
        #HOSP They then are mirrored and extruded up. So we now have a square drawn on one layer as a pillar.
       
        
        my @pillars = ();
        #HOSP  Here a pillars array is created
        #HOSP The code below is used for positioning the pillars.
        my $bb = Slic3r::Geometry::BoundingBox->new_from_points([ map @$_, map @$_, values %$contact ]);
        #HOSP Here the points are sent through the bounding box section to make sure they are in the area smaller then the
        #HOSP contact points I think.
        ...
        print STDERR "Bounding Box y max";
        print STDERR $bb->y_max;
        print STDERR "////////////////////////////////";
        for (my $x = $bb->x_min; $x <= $bb->x_max-$pillar_size; $x += $pillar_spacing) {
        # for (my $x = $bb->x_min; $x <= $bb->x_max-x_max/10; $x += x_max/10) {
            print STDERR  $x_max_halved;
            $counting_one+=1;
            print STDERR "count";
            print STDERR $counting_one;
            
            # my $internal_x_max_halved = $bb->x_max/2;
            # print STDERR "X_MAX_HALVED";
            # my $internal_x_max_h_squared = sqrt($x_max_halved);
            # print STDERR $internal_x_max_halved;
            # print STDERR "X_MAX_H_SQUARED";
            # print STDERR $internal_x_max_h_squared;
            # my $internal_x_max_pillar = $internal_x_max_h_squared/1000;
            # print STDERR $internal_x_max_pillar;
            # my $internal_x_max_pillar_spacing = $internal_x_max_pillar+1;
            ...
            print STDERR "//////";
            #HOSP Bounding box x_max ($bb->x_max) is the maximum width of the object for support material generation + 001775539. So if the max width is 130, the x_max would be 131775539
            #HOSP This is the same for y_max

            #HOSP So create something which mean that the pillar size is vairable judged on the size of the x_max and x_min bounding box
            #HOSP I then want to create a feature in which the area of contact also influcenes the placemnt of the pillar and then furthermore defines when the model can be extrued up
            #HOSP I then want to allow for diffrent levels of the tree to be extruded out towards certai contact arreas

            #HOSP So here X is x_min and is smaller then x - pillar size and then x is added the pillar spacing. 
            for (my $y = $bb->y_min; $y <= $bb->y_max-$pillar_size; $y += $pillar_spacing) {
                ...
                print STDERR "count";
                print STDERR $counting_one;
                #HOSP P is a pillar clone and is simply pushed to the Pillars array, unrelated to the x y code
                ...
                print STDERR "Y";
                print STDERR $y;
                print STDERR "X HALVED";
                $x_max_halved-=$x;
                print STDERR $x_max_halved;
                print STDERR "X HALVED";
                #HOSP P is then moved using the x y positioning
            }
        }
        #HOSP print STDERR @pillars;
        $grid = union(\@pillars);
    }
    ...
    #HOSP So goes through each layer and appends the grid array to each layer of the shape. 
    
    # build capitals
    for my $i (0..$#$support_z) {
        my $z = $support_z->[$i];
        #HOSP Goes through each layer and z is the first something? in that layer either point or shape
        my $capitals = intersection(
            $grid,
            $contact->{$z} // [],
        );
        
        #HOSP Here the you find out which numbers from grid are also in contact  using the intersect tool which 
        #HOSP checks for matching numbers in two arrays. Adds to capitals array.
        ...
        #HOSP Here is an array for the capitals 
        foreach my $capital (@$capitals) {
            #HOSP For each capital in the capitals array.
            # enlarge capital tops
            $capital = offset([$capital], +($pillar_spacing - $pillar_size)/2);
            #HOSP So here I belieive it looks like this is the code for expanding the tops?
            push @$contact_supported_by_capitals, @$capital;
            #HOSP Push these capitals to Contact_supported_by_captials array
            for (my $j = $i-1; $j >= 0; $j--) {
                #HOSP Creat what appears to be an infinite loop
                my $jz = $support_z->[$j];
                #HOSP My jz is the support z for the loop number
                $capital = offset($capital, -$self->interface_flow->scaled_width/2);
                
                #HOSP Interface slow has something to do with the toolpath and it expands by scaled width/2
                #HOSP The division of scaled width shows how quickly it expands, the smaller the number the lower and quciker expansion
                #HOSP of capitals.
                last if !@$capitals;
                push @{ $shape->[$j] }, @$capital;
                #HOSP push to capitals
            }
        }
        ...
        #HOSP Here you are defining which contact areas arent supported

        if (@$contact_not_supported_by_capitals) {
            for (my $j = $i-1; $j >= 0; $j--) {
                #HOSP push @{ $shape->[$j] }, @$contact_not_supported_by_capitals;
                #HOSP COMMENTEDOUT 
                #HOSP in a loop your pushing the the contact not supported capitals to shape to be generated
            }
        }
    }
}
